...
export function sha256HexBytes(input) {
  ...
  const paddedByte = (position) => {
    if (position < bytes.length) return bytes[position];
    if (position === bytes.length) return 0x80;
    if (position >= paddedLength - 8) {
      const lengthIndex = position - (paddedLength - 8);
      const value = lengthIndex < 4 ? bitLengthHigh : bitLengthLow;
      return (value >>> ((3 - (lengthIndex % 4)) * 8)) & 0xff;
    }
    return 0;
  };
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    for (let index = 0; index < 16; index += 1) {
      ...
      words[index] = (
        (paddedByte(position) << 24)
        | (paddedByte(position + 1) << 16)
        | (paddedByte(position + 2) << 8)
        | paddedByte(position + 3)
      ) >>> 0;
    }
