import aiModelConfig from "../../config/ai-models.json";
import {
...
} from "./standalone-default-scenario.js";
...
const API_CONFIG_KEY = "night-mailbox-mobile-api-config";
...
const mobileDefaultSettings = {
  ...
  ensemble: {
    ...
    friend: {
      ...
      relation: "晚晚的成年闺蜜",
      prompt: "说话爽快自然，善于活跃气氛，也会认真照顾朋友的感受。",
      appearance: "成年女性，清爽自然、亲切有活力。",
      imagePrompt: "",
      avatarUrl: "",
    },
    customRoles: [],
...
const downstream = aiModelConfig.downstream || {};
const defaultApiConfig = {
  deepseekBaseUrl: "https://api.deepseek.com",
  deepseekModel: "deepseek-v4-flash",
  deepseekKey: "",
  chatStream: true,
  downstreamBaseUrl: String(downstream.baseUrl || "").replace(/\/$/, ""),
  downstreamKey: "",
  imageBaseUrl: String(downstream.image?.baseUrl || "").replace(/\/$/, ""),
  imageKey: "",
};
...
export async function initializeMobileStorage() {
  ...
  const keys = [SETTINGS_KEY, HISTORY_KEY, IMAGE_JOBS_KEY, API_CONFIG_KEY];
...
export function getMobileApiConfig() {
  const stored = readStoredJson(API_CONFIG_KEY, defaultApiConfig);
  return {
    ...defaultApiConfig,
    ...stored,
    chatStream: stored?.chatStream !== false && stored?.chatStream !== "false",
  };
}
...
export function saveMobileApiConfig(value) {
  ...
  for (const key of Object.keys(next)) {
    if (key === "chatStream") continue;
    next[key] = String(next[key] || "").trim();
  }
  storageSet(API_CONFIG_KEY, next);
  return next;
}
...
function getProvider(body) {
  ...
  const isGrok = body?.provider === "grok";
  const configuredDeepSeekModel = String(config.deepseekModel || "deepseek-v4-flash");
  return isGrok
    ? {
        id: "grok",
        key: config.downstreamKey,
        baseUrl: config.downstreamBaseUrl,
        model: String(body?.model || downstream.chat?.defaultModel || "grok-4.5"),
    ...
    : {
        ...
        model: ["deepseek-chat", "deepseek-reasoner"].includes(configuredDeepSeekModel)
          ? "deepseek-v4-flash"
...
async function callChatModel(body, messages, options = {}) {
  ...
  if (!content || incompleteStream) {
    ...
    if (!response.ok) {
      throw modelResponseError(
        `${provider.id === "deepseek" ? "DeepSeek" : "Grok"} 响应恢复失败（${response.status}）：${response.text.slice(0, 500)}`,
        response,
    ...
    if (!result || typeof result !== "object") {
      throw modelResponseError(
        `${provider.id === "deepseek" ? "DeepSeek" : "Grok"} 恢复请求返参不是有效 JSON`,
        response,
...
示例输出：
{"scene":"共享场景","turns":[{"speaker":"晚晚","scene":"角色所在场景","mood":"心情","action":"动作","dialogue":"台词","progression":""},{"speaker":"小雨","scene":"","mood":"心情","action":"动作","dialogue":"台词","progression":"两人已经开始执行的下一步行动及其直接影响"}]}
...
async function handleChat(body) {
  ...
  try {
    ...
    const providerName = selectedProvider.id === "deepseek" ? "DeepSeek" : "Grok";
    const fallbackText = `刚才 ${providerName} 连续返回了空白或不完整内容。我先停在这里，不会让角色自行继续。你可以重新发送刚才的话，我会从当前剧情接着回复。`;
    if (!multiMessageMode) {
      return textResponse(fallbackText, 200, {
        "X-Chat-Provider": body.provider || "deepseek",
...
async function handleWorld(body) {
  ...
  return jsonResponse({ worldSetting: content.slice(0, 12000), provider: body.provider || "deepseek" });
}
...
normal 表示实际和外表每过365剧情天增加一岁；fixed 表示实际年龄增加但外表固定；long-lived 表示长生种实际年龄增加、外表缓慢或固定；ageless 表示两者均不变。资料不明确时用 null 或 unknown，不要强行改成成年人。`,
  ...
  return jsonResponse({ role: parsed, provider: body.provider || "deepseek" });
}
...
async function prepareImagePrompt(body, kind) {
  ...
  return jsonResponse({
    ...
    model: downstream.image?.defaultModel || "gpt-image-2",
    quality: "standard",
    size: downstream.image?.portraitSize || "1024x1536",
  });
}
...
${transcript}`,
  ...
  return jsonResponse({
    ...
    model: downstream.image?.defaultModel || "gpt-image-2",
    size: downstream.image?.landscapeSize || "1536x1024",
  });
}
...
function safeBackupPreferences() {
  try {
    ...
    return {
      chatProvider: stored.chatProvider === "grok" ? "grok" : "deepseek",
      grokModel: typeof stored.grokModel === "string" ? stored.grokModel.slice(0, 100) : "",
...
async function runImageJob(id, body) {
  ...
  try {
    ...
    const imageUrl = await downloadGeneratedImage(remoteUrl, filename);
    updateImageJob(id, {
      status: "completed",
...
async function routeMobileRequest(path, method, body, url) {
  ...
  if (path === "/api/health") {
    const config = getMobileApiConfig();
    return jsonResponse({
      chat: config.deepseekKey ? "configured" : "demo",
      grok: config.downstreamKey ? "configured" : "missing",
      deepseekModel: config.deepseekModel || "deepseek-v4-flash",
      grokModel: downstream.chat?.defaultModel || "grok-4.5",
      image: config.imageKey ? "configured" : "missing",
      imageModel: downstream.image?.defaultModel || "gpt-image-2",
      mobile: true,
    });
  }
  if (path === "/api/models") {
    const config = getMobileApiConfig();
    let discovered = [];
    if (config.downstreamKey && config.downstreamBaseUrl) {
      try {
        const response = await nativeHttpRequest(`${config.downstreamBaseUrl}/models`, {
          method: "GET",
          headers: { Authorization: `Bearer ${config.downstreamKey}` },
          timeout: 45000,
        });
        if (response.ok) {
    ...
    const models = [...new Set([...(downstream.chat?.models || []), ...discovered])];
    return jsonResponse({
      models,
      defaultModel: downstream.chat?.defaultModel || downstream.chat?.models?.[0] || "",
      source: discovered.length ? "direct-api" : "local-config",
      authConfigured: Boolean(config.downstreamKey),
    });
  }
...
export function mountMobileApiSettings() {
  const backdrop = document.createElement("div");
  backdrop.className = "mobile-api-backdrop";
  backdrop.innerHTML = `
    <section class="mobile-api-sheet" role="dialog" aria-modal="true">
      <h2>本地 API 设置</h2>
      <p>密钥和全部资料只保存在当前浏览器的 IndexedDB。模型与图片接口由本页面直接调用，不经过电脑或 Node 服务。</p>
      <label>DeepSeek API Key<input data-key="deepseekKey" type="password" autocomplete="off" placeholder="用于总结、提示词和 DeepSeek 对话"></label>
      <label>DeepSeek 地址<input data-key="deepseekBaseUrl" type="url"></label>
      <label>DeepSeek 模型<input data-key="deepseekModel" type="text"></label>
      <label class="mobile-api-toggle">
        <span><b>对话流式传输</b><small>开启时请求使用 stream:true；关闭时使用普通 JSON 返回。界面均在整轮完成后展示。</small></span>
        <input data-key="chatStream" type="checkbox">
      </label>
      <label>Grok / Claude 中转 Key<input data-key="downstreamKey" type="password" autocomplete="off"></label>
      <label>Grok / Claude 中转地址<input data-key="downstreamBaseUrl" type="url"></label>
      <label>图片生成 Key<input data-key="imageKey" type="password" autocomplete="off"></label>
      <label>图片接口地址<input data-key="imageBaseUrl" type="url"></label>
      <div class="mobile-api-actions">
        <button class="mobile-api-cancel" type="button">取消</button>
        <button class="mobile-api-save" type="button">保存并刷新</button>
      </div>
    </section>`;
  ...
  if (!getMobileApiConfig().deepseekKey && !getMobileApiConfig().downstreamKey) open();
}
