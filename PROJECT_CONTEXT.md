...
1. 生图**不自动重试**：每次提交只调用一次图片接口，失败即标记任务失败并给出错误提示。
2. 不再使用“400 内容拒绝 → 对话模型改写提示词 → 再次调用”的自动重试链路，避免重复扣费。
3. 任务记录保存 `attempt`、`maxAttempts`、`prompt`、`rewritten`、`statusMessage` 与失败时的 `error`/`code`，便于排查。
4. 超时、鉴权、余额、网络、5xx 和下载失败直接呈现给用户，不静默重试。
...
5. 不把场景生图重新塞回聊天页面。
6. 场景和角色生图继续使用后台任务。
7. 生图失败不自动重试：每次只调用一次图片接口，失败直接标记任务并提示错误。
...
- 批量生图会先显示张数和按每张约 0.03 元计算的估算费用，确认后才提交。Node 与单文件 HTML/App 共用“最多 6 张并发”的后台任务规则：队列有空位即同时启动，超过 6 张的部分继续排队；每张图只调用一次图片接口，失败即标记任务。`mobile/web/native-api.js` 的队列和任务记录持久化到 IndexedDB；动作图使用 `kind: "visual-state"`，不会混入场景相册，也不会自动覆盖角色头像。
